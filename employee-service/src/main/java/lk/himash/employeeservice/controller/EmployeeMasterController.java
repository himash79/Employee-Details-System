...
public class EmployeeMasterController {
    ...
    @PutMapping("/v1/updateEmployee/{empID}")
    public ResponseEntity<?> updateEmp(@RequestBody EmployeeMasterDto newEmpMaDto, @PathVariable String empID) {
    	System.out.println("Start | updateEmp() method | EmployeeMasterController.class");
        System.out.println("Parameters --> empID : " + empID);
    	ResponseEntity resp;
    	try {
    		EmployeeMaster empObject = DtoConvertor.toEmpMaEntity(newEmpMaDto);
    		resp = empMaService.updateEmployee(empObject, empID);
    	} catch(Exception ex) {
    		System.out.println("Exception found on | updateEmp() method | EmployeeMasterController.class");
            System.out.println(ex.getMessage());
            return new ResponseEntity("ERROR", HttpStatus.BAD_REQUEST);
    	}
    	System.out.println("Successfully delete employee details");
